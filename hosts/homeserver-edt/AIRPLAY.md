...
| `basic_string: construction from null is not valid` at startup, core dump | Upstream bug in `-scrsv`: it builds a `std::string` from `getenv("XDG_CURRENT_DESKTOP")` without checking it is set, so UxPlay aborts when started outside a desktop session. The wrapper now defaults `XDG_RUNTIME_DIR`, `DBUS_SESSION_BUS_ADDRESS`, `XDG_CURRENT_DESKTOP`, `DISPLAY` and `XAUTHORITY` (bisected: `DESKTOP_SESSION` and `XDG_SESSION_TYPE` make no difference). |
